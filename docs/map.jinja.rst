...
We will see a quick setup to configure the ``nftables`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``nftables`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``nftables`` formulas under ``/srv/salt/nftables/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/nftables/parameters/dns:domain/
    ln -s dns:domain /srv/salt/nftables/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/nftables/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/nftables/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``nftables`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``nftables`` formulas under ``/srv/salt/nftables/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/nftables/parameters/roles
...
We create a configuration for the role ``nftables/server`` in ``/srv/salt/nftables/parameters/roles/nftables/server.yaml``:
...
We create another configuration for the role ``nftables/client`` in ``/srv/salt/nftables/parameters/roles/nftables/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles nftables/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles nftables/client
...
.. code-block:: console
    ...
    mv /srv/salt/nftables/parameters/dns:domain/  '/srv/salt/nftables/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-nftables-as-json:
      test.nop:
        - name: {{ nftables | json }}
...
Here is an example based on `template-formula/nftables/config/file.sls`_:
    ...
    nftables-config-file-file-managed:
      file.managed:
        ...
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='nftables-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ nftables.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``nftables`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ nftables['winner'] }}
...
.. _template-formula/nftables/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/nftables/config/file.sls
